...
export class WhatsAppAIService extends AIService {
  ...
  public async generateWhatsAppResponse(
    ...
    try {
      ...
      if (context.messageWindow === 'EXPIRED') {
        aiResponse.templateSuggestions = [await this.generateTemplateMessage(
          customerMessage,
          context.merchantId
        )];
      }
  ...
  private getWhatsAppContextualFallback(
    ...
    const ERROR_MAP: Record<string, string> = { RATE_LIMIT: 'شكراً لتواصلك معنا. سنرد عليك خلال ساعات العمل. 🕐', AI_API_ERROR: 'نعتذر عن التأخير. فريق خدمة العملاء سيتواصل معك خلال دقائق.', NETWORK_ERROR: 'حدث خطأ تقني مؤقت. يرجى إعادة إرسال الرسالة خلال دقائق قليلة.' };
    const message = ERROR_MAP[errorType] ?? ERROR_MAP['AI_API_ERROR'];
    ...
    return {
      ...
      stage: context.stage,
      actions: [{ 
        type: context.messageWindow === 'EXPIRED' ? 'ESCALATE' : 'ESCALATE', 
...
🎯 يجب أن تكون إجابتك بصيغة JSON:
{
  ...
  private async generateTemplateText(intent: string, merchantId: string): Promise<string> {
    const templates: Record<string, string> = { greeting: 'مرحباً {{1}}! شكراً لتواصلك مع {{2}}. كيف يمكننا مساعدتك؟', product_inquiry:'مرحباً! نشكرك على استفسارك عن منتجاتنا. تفضل بزيارة متجرنا أو راسلنا للمزيد من التفاصيل.', order_confirmation:'شكراً لطلبك! سنتواصل معك قريباً لتأكيد التفاصيل والتوصيل.', support:'', error:'' };
