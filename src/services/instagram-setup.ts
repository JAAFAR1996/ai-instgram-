...
export class InstagramSetupService {
  ...
  public async validateBusinessAccount(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        issues.push(`API Error: ${error.error?.message || 'Invalid credentials'}`);
        return { isValid: false, issues };
      }

      const accountInfo: BusinessAccountInfo = await response.json();
  ...
  private async testInstagramAPI(config: InstagramSetupConfig): Promise<BusinessAccountInfo> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(`Instagram API test failed: ${error.error?.message || 'Unknown API error'}`);
    }
