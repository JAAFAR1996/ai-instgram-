...
export class InstagramMessageSender {
  ...
  public async sendTemplateMessage(
    ...
    try {
      ...
      const response = await client.sendMessage({
        recipientId,
        message: '', // Template content is in the template field
  ...
  private async uploadMedia(
    ...
    try {
      ...
      const uploadResponse = await client.uploadMedia(mediaPath, mediaType);
  ...
  private async checkMessageWindow(
    ...
    try {
      const windowStatus = await this.messageWindowService.checkWindow(
        merchantId,
        { instagram: recipientId, platform: 'INSTAGRAM' }
      );
