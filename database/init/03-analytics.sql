...
WITH merchant_stats AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN p.stock_quantity <= p.min_stock_alert THEN p.id END) as low_stock_products,
...
WITH daily_stats AS (
    SELECT 
        ...
        (SELECT COUNT(*) FROM orders WHERE DATE(created_at) = DATE(m.created_at)) as daily_orders,
        (SELECT COUNT(*) FROM orders WHERE DATE(created_at) = DATE(m.created_at) AND status = 'CONFIRMED') as daily_confirmed_orders,
        (SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE DATE(created_at) = DATE(m.created_at) AND status IN ('CONFIRMED', 'DELIVERED')) as daily_revenue,
        
        -- Conversation Statistics
        (SELECT COUNT(*) FROM conversations WHERE DATE(created_at) = DATE(m.created_at)) as daily_conversations,
        (SELECT COUNT(*) FROM conversations WHERE DATE(created_at) = DATE(m.created_at) AND converted_to_order = true) as daily_converted_conversations,
        
        -- Message Statistics
        (SELECT COUNT(*) FROM message_logs WHERE DATE(created_at) = DATE(m.created_at)) as daily_messages,
        (SELECT COUNT(*) FROM message_logs WHERE DATE(created_at) = DATE(m.created_at) AND direction = 'INCOMING') as daily_incoming_messages,
        (SELECT COUNT(*) FROM message_logs WHERE DATE(created_at) = DATE(m.created_at) AND direction = 'OUTGOING') as daily_outgoing_messages
...
SELECT 
    *,
    CASE 
        WHEN daily_orders > 0 THEN ROUND((daily_confirmed_orders::DECIMAL / daily_orders) * 100, 2)
        ELSE 0 
    END as daily_order_confirmation_rate,
    
    CASE 
        WHEN daily_conversations > 0 THEN ROUND((daily_converted_conversations::DECIMAL / daily_conversations) * 100, 2)
        ELSE 0 
...
SELECT 
    *,
    ...
    CASE 
        WHEN stock_quantity + total_sold > 0 THEN ROUND((total_sold::DECIMAL / (stock_quantity + total_sold)) * 100, 2)
        ELSE 0 
